...
def wrap_lines(lines, maxlength=80):
    ...
    for line in lines:
        ...
        for old_line_part in map(str, line.split(" ")):
            if (len(old_line_part) + sum(len(w) + 1 for w in new_line_parts)) >= maxlength:
...
def update_puzzle_text(filepath, num, cookies):
    ...
    Args:
        ...
        cookies (dict): nom
    """
    response = requests.get(f"https://adventofcode.com/{YEAR:d}/day/{num:d}", cookies=cookies)
    ...
    with filepath.open(mode="w") as f:
        for part in (i.contents for i in soup.find_all("article")):
            html_as_text = "\n".join(tag_to_text(i) if hasattr(i, "text") else i.rstrip() for i in part).splitlines()
...
def get_puzzle_input(filepath, num, cookies):
    ...
    Args:
        ...
        cookies (dict): nom
    """
    r = requests.get(f"https://adventofcode.com/{YEAR:d}/day/{num:d}/input", cookies=cookies)
...
if __name__ == "__main__":
    ...
    if not authfile.exists():
        _cookies = {"_ga": input("Enter cookie_ga: ").strip(), "session": input("Enter cookie_session: ").strip()}
